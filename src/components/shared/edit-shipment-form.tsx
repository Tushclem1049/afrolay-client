...
const EditShipmentForm = ({ shipment }: { shipment: TShipment }) => {
  const {
    states: {
      // isModalOpen,
      isSubmitting,
      shipment: { belongsTo, destination, events, status, origin, trackingId },
      // shipmentEvent,
      // whatToDo,
    },
    actions: {
  ...
  return (
    <form onSubmit={(e) => handleShipmentUpdate(e, trackingId)}>
      <p>
        <BsInfoCircle />
      ...
      <div>
        ...
        >
          {isSubmitting ? "Please wait..." : "Save Shipment"}
        </button>
      </div>
